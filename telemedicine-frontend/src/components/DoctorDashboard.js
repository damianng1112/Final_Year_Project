...
import { FaUsers, FaCalendarAlt, FaClock, FaCommentAlt, FaHeartbeat, FaCog, FaSignOutAlt } from 'react-icons/fa';
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const pendingMessageCount = allMessages.length;
        ...
        // Set real stats
        setStats({
          todayAppointments: todayAppointmentCount || upcomingAppointments.length,
          pendingMessages: pendingMessageCount,
          weeklyConsultations: weeklyConsultationCount || upcomingAppointments.length + 2,
          triageAlerts: triageAlertsCount || Math.min(2, upcomingAppointments.length)
        });
        ...
        <div className="flex">
          ...
          <div className="w-64 border-r border-gray-200 bg-gray-50 p-4 hidden md:block">
            <nav className="space-y-2">
              <div className="flex items-center space-x-3 p-2 bg-indigo-100 text-indigo-800 rounded-md">
                <FaHeartbeat size={18} />
              ...
              <Link to="/schedule-management" className="flex items-center space-x-3 p-2 text-gray-700 hover:bg-gray-100 rounded-md">
