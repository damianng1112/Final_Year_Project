...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (role === 'doctor' && !doctorId) {
      setError('Please provide all required doctor details.');
      return;
    }
      ...
      <div className="w-full max-w-lg p-8 space-y-6 bg-white shadow-md rounded-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {role === 'doctor' && (
            <>
              <div>
                <label htmlFor="doctorId" className="block text-sm font-medium text-gray-700">Doctor ID (for verification)</label>
                <input
                  type="text"
                  id="doctorId"
                  value={doctorId}
